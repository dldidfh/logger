...
class CallLog:
    ...
    def __init__(self, log_name, log_level : Union[str, int]) -> None:
        self._logger = logging.getLogger(log_name)
        self._file_handler = None 
        self._stream_handler = None 
        self._root_logger_log_level = self._call_lever( log_level )
    ...
    @staticmethod
    def _call_lever(level:Union[int, str]):
    ...
    def add_file_handler(self, path:str, H_type=None, when="midnight", interval=1, log_level:Union[str,int]=None, name="file_handler") -> None:
        if H_type == "rotating" or H_type == "R":
            handler = TimedRotatingFileHandler(filename=f'{path}_{str(datetime.now().strftime(CallLog._TIME_FORMAT))}.log', encoding='utf-8', when=when, interval=interval)
        else: 
            handler = logging.FileHandler(filename=f'{path}_{str(datetime.now().strftime(CallLog._TIME_FORMAT))}.log',mode='a', encoding='utf-8')
        if log_level != None :
            log_level = self._call_lever(log_level)
